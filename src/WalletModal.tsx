...
const WalletModal = ({ isOpen, onClose, onSelect }: WalletModalProps) => {
  ...
  return (
    <Modal close={onClose} open={isOpen} title="Connect Wallet">
      <div className="grid grid-cols-2 items-center justify-between flex-row flex-wrap w-[35vw] mx-auto p-8 rounded-lg bg-purple-50">
        {wallets.map((wallet) => {
          return (
            <button
              key={wallet.id}
              onClick={() => onSelect(wallet.id)}
              className="flex flex-col items-center justify-between w-full p-4 hover:bg-purple-100 rounded-[8%] text-purple-900"
            >
